...
def write_scraped_data(data):
    ...
    wb = load_workbook(excel_file_path, keep_vba=True)  # Open workbook without 'with'
    try:
...
class YellowPageScraper:
    ...
    def __init__(self, search_terms, geo_location_terms, start_page, max_pages):
        ...
        try:
            self.wb = load_workbook(excel_file_path, keep_vba=True)  # Load the workbook
...
if __name__ == "__main__":
    try:
        ...
        # Ensure that completion status is written to the file
        try:
